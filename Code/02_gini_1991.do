****************************************************
* 02_gini_1991.do
* Build 1991 consumption (core/broad) and Gini
****************************************************

clear all
set more off

global intr  "/Volumes/SSD PRO/Github-forks/Chp3-EnergyIneq/Code/Data/CEX/intrvw91"
...
* 1991 MTBI files: mtbi911-914 (flat structure)
use mtbi911.dta, clear
foreach f in mtbi912 mtbi913 mtbi914 {
    append using `f'.dta
}
...
****************************************************
merge m:1 UCC using "$deriv/ucc_map_1991.dta"
keep if _merge==3
drop _merge
...
****************************************************
save "$deriv/mtbi_1991_tagged.dta", replace
...
save "$deriv/income_1991_cuq.dta", replace
...
****************************************************
use "$deriv/mtbi_1991_tagged.dta", clear
...
save "$deriv/cons_1991_cuq.dta", replace
...
save "$deriv/cons_1991_cuy.dta", replace

****************************************************
* 8. Compute quarterly consumption Ginis
****************************************************
postfile gini_post int year int quarter double gini_core double gini_broad ///
    using "$deriv/gini_1991_quarterly.dta", replace

forvalues q = 1/4 {
    quietly {
        use "$deriv/cons_1991_cuq.dta", clear
        keep if quarter == `q'
...
postfile gini_inc_post int year int quarter double gini_fincbtax double gini_fsalaryx ///
    using "$deriv/gini_1991_income_quarterly.dta", replace

forvalues q = 1/4 {
    quietly {
        use "$deriv/income_1991_cuq.dta", clear
        keep if quarter == `q'
    ...
    post gini_inc_post (1991) (`q') (g_fincbtax) (g_fsalaryx)
}
...
****************************************************
use "$deriv/cons_1991_cuy.dta", clear
...
save "$deriv/gini_1991_annual.dta", replace

****************************************************
* 10. Combine quarterly + annual
****************************************************
* Merge consumption and income quarterly Ginis
use "$deriv/gini_1991_quarterly.dta", clear
merge 1:1 year quarter using "$deriv/gini_1991_income_quarterly.dta"
drop _merge

* Append annual consumption Ginis (no annual income Ginis)
append using "$deriv/gini_1991_annual.dta"
...
save "$deriv/gini_1991_all.dta", replace

* Clean up intermediate files
erase "$deriv/gini_1991_quarterly.dta"
erase "$deriv/gini_1991_income_quarterly.dta"
erase "$deriv/gini_1991_annual.dta"

display _n "Saved: gini_1991_all.dta"
