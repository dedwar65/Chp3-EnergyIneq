****************************************************
* 06_toggle_weights.do
* Toggle: Frequency Weights (fw = round(FINLWT21/3))
*
* Inputs:
...
*
* Output:
*   - $robust_data/gini_rr_merged_weights.dta
*
* Methodology:
*   - Use frequency weights: fw = round(FINLWT21/3)
...
display _n "========================================"
display "TOGGLE: Frequency Weights (fw = round(FINLWT21/3))"
display "========================================" _n

****************************************************
* 1. Compute Ginis with frequency weights
****************************************************
display _n "=== Computing Ginis with Frequency Weights (1990-2008) ==="
...
forvalues y = 1990/2008 {
    forvalues q = 1/4 {
        quietly {
            ...
            * Create frequency weights: fw = round(FINLWT21/3)
            gen fwt = round(FINLWT21/3)
            
            * Compute consumption Ginis with frequency weights
            scalar gc = .
            scalar gb = .
            
            if _N > 0 {
                capture ineqdeco cons_core_q [fw = fwt]
                if !_rc {
                    scalar gc = r(gini)
                }
                
                capture ineqdeco cons_broad_q [fw = fwt]
                if !_rc {
                    scalar gb = r(gini)
                }
            }
            ...
            * Load income data
            ...
            if !_rc {
                ...
                * Compute income Ginis (exclude zeros/missing) with frequency weights
                preserve
                drop if missing(fincbtax) | fincbtax <= 0
                if _N > 0 {
                    capture ineqdeco fincbtax [fw = fwt]
                    if !_rc {
                ...
                if _N > 0 {
                    capture ineqdeco fsalaryx [fw = fwt]
                    if !_rc {
...
****************************************************
sort year quarter
save "$robust_data/gini_rr_merged_weights.dta", replace

display _n "========================================"
display "TOGGLE COMPLETE: Frequency Weights"
display "========================================"
display "Saved: $robust_data/gini_rr_merged_weights.dta"
...
use "$robust_data/gini_rr_merged_weights.dta", clear
