****************************************************
* 07_toggle_winsor.do
* Toggle: Winsorization (1st and 99th percentiles)
*
* Inputs:
...
*
* Output:
*   - $robust_data/gini_rr_merged_winsor.dta
*
* Methodology:
*   - Winsorize consumption/income at 1st and 99th percentiles
...
display _n "========================================"
display "TOGGLE: Winsorization (1st/99th percentiles)"
display "========================================" _n

****************************************************
* 1. Compute Ginis with winsorization
****************************************************
display _n "=== Computing Ginis with Winsorization (1990-2008) ==="
...
forvalues y = 1990/2008 {
    forvalues q = 1/4 {
        quietly {
            ...
            * Winsorize at 1st and 99th percentiles (using frequency weights)
            gen fwt = round(FINLWT21/3)
            
            * Winsorize cons_core_q
            quietly summarize cons_core_q [fw = fwt], detail
            local p1_core = r(p1)
            local p99_core = r(p99)
            replace cons_core_q = `p1_core' if cons_core_q < `p1_core' & cons_core_q != .
            replace cons_core_q = `p99_core' if cons_core_q > `p99_core' & cons_core_q != .
            
            * Winsorize cons_broad_q
            quietly summarize cons_broad_q [fw = fwt], detail
            local p1_broad = r(p1)
            local p99_broad = r(p99)
            replace cons_broad_q = `p1_broad' if cons_broad_q < `p1_broad' & cons_broad_q != .
            replace cons_broad_q = `p99_broad' if cons_broad_q > `p99_broad' & cons_broad_q != .
            ...
            * Load income data
            ...
            if !_rc {
                ...
                * Create frequency weights for winsorization
                gen fwt = round(FINLWT21/3)
                
                * Winsorize income (only for non-missing, positive values)
                preserve
...
****************************************************
sort year quarter
save "$robust_data/gini_rr_merged_winsor.dta", replace

display _n "========================================"
display "TOGGLE COMPLETE: Winsorization"
display "========================================"
display "Saved: $robust_data/gini_rr_merged_winsor.dta"
...
use "$robust_data/gini_rr_merged_winsor.dta", clear
