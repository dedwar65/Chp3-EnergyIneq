...
****************************************************
* 5. Collapse to CU-quarter level
****************************************************
collapse (sum) cons_core_q=cons_core_item cons_broad_q=cons_broad_item, by(NEWID quarter)
gen year = 2023
order NEWID year quarter
save "$deriv/cons_2023_cuq.dta", replace

****************************************************
* 6. Collapse to CU-year level
****************************************************
collapse (sum) cons_core_y=cons_core_q cons_broad_y=cons_broad_q, by(NEWID)
gen year = 2023

****************************************************
* 7. Merge weights from FMLI
****************************************************
cd "$intr"
...
merge 1:1 NEWID using `weights'
keep if _merge==3
drop _merge

* Drop zero/negative consumption
drop if cons_core_y <= 0 | cons_broad_y <= 0
...
scalar gini_core_2023  = r(gini)

ineqdeco cons_broad_y [aw = FINLWT21]
scalar gini_broad_2023 = r(gini)

****************************************************
* 9. Save year-level summary
****************************************************
clear
set obs 1
gen year       = 2023
gen gini_core  = gini_core_2023
gen gini_broad = gini_broad_2023
save "$deriv/gini_2023.dta", replace
...
display "  $deriv/gini_2023.dta (Gini summary)"
