****************************************************
* 02_gini_1995.do
* Build 1995 consumption (core/broad) and Gini
****************************************************

clear all
set more off

global intr  "/Volumes/SSD PRO/Github-forks/Chp3-EnergyIneq/Code/Data/CEX/intrvw95/intrvw95"
...
* 1995 MTBI files: mtbi951-954 (mtbi961 is used for 1996)
use mtbi951.dta, clear
foreach f in mtbi952 mtbi953 mtbi954 {
    append using `f'.dta
}
...
****************************************************
merge m:1 UCC using "$deriv/ucc_map_1995.dta"
keep if _merge==3
drop _merge
...
****************************************************
save "$deriv/mtbi_1995_tagged.dta", replace
...
save "$deriv/income_1995_cuq.dta", replace
...
****************************************************
use "$deriv/mtbi_1995_tagged.dta", clear
...
save "$deriv/cons_1995_cuq.dta", replace
...
save "$deriv/cons_1995_cuy.dta", replace

****************************************************
* 8. Compute quarterly consumption Ginis
****************************************************
postfile gini_post int year int quarter double gini_core double gini_broad ///
    using "$deriv/gini_1995_quarterly.dta", replace

forvalues q = 1/4 {
    quietly {
        use "$deriv/cons_1995_cuq.dta", clear
        keep if quarter == `q'
...
postfile gini_inc_post int year int quarter double gini_fincbtax double gini_fsalaryx ///
    using "$deriv/gini_1995_income_quarterly.dta", replace

forvalues q = 1/4 {
    quietly {
        use "$deriv/income_1995_cuq.dta", clear
        keep if quarter == `q'
    ...
    post gini_inc_post (1995) (`q') (g_fincbtax) (g_fsalaryx)
}
...
****************************************************
use "$deriv/cons_1995_cuy.dta", clear
...
save "$deriv/gini_1995_annual.dta", replace

****************************************************
* 10. Combine quarterly + annual
****************************************************
* Merge consumption and income quarterly Ginis
use "$deriv/gini_1995_quarterly.dta", clear
merge 1:1 year quarter using "$deriv/gini_1995_income_quarterly.dta"
drop _merge

* Append annual consumption Ginis (no annual income Ginis)
append using "$deriv/gini_1995_annual.dta"
...
save "$deriv/gini_1995_all.dta", replace

* Clean up intermediate files
erase "$deriv/gini_1995_quarterly.dta"
erase "$deriv/gini_1995_income_quarterly.dta"
erase "$deriv/gini_1995_annual.dta"

display _n "Saved: gini_1995_all.dta"
